...
export default function MyPage(): JSX.Element {
  ...
  const getUserData = async () => {
    try {
      ...
      const response = await axios.get(`/api/auth/profile?email=${email}`);
  ...
  const handleAcceptRequest = (requestId: string) => {
    ...
    const updatedRequests = profileData.friendRequests.filter(
      (r) => r.id !== requestId,
    );
  ...
  const handleRemoveFriend = (friendId: string) => {
    if (confirm("Are you sure you want to remove this friend?")) {
      const updatedFriends = profileData.friends.filter(
        (f) => f.id !== friendId,
      );
  ...
  const handleAddFriend = () => {
    ...
    if (
      friendsDisplay.some(
        (f) => f.username.toLowerCase() === newFriendUsername.toLowerCase(),
      )
    ) {
    ...
    if (
      friendRequestsDisplay.some(
        (r) => r.username.toLowerCase() === newFriendUsername.toLowerCase(),
      )
    ) {
    ...
    alert(
      `Friend request sent to ${newFriendUsername}! (For demo purposes, it appears as an incoming request)`,
    );
  };
              ...
              {friendsDisplay.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-400 text-sm">
                    Add friends to connect with other collectors
                  </p>
                </div>
              ) : (
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                ...
                <p className="text-xs text-blue-800">
                  ...
                  They will receive a friend request.
                </p>
              </div>
            </div>
